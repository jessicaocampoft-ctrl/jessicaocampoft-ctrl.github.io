...
function constantTimeEqual_(a, b) {
  ...
  for (var i = 0; i < length; i++) difference |= (i < a.length ? a.charCodeAt(i) : 0) ^ (i < b.length ? b.charCodeAt(i) : 0);
  return difference === 0;
}

function cloudflareSignature_(message, secret) {
  var bytes = Utilities.computeHmacSha256Signature(message, secret, Utilities.Charset.UTF_8);
  return Utilities.base64EncodeWebSafe(bytes).replace(/=+$/, '');
}

function createCloudflareSession_(p) {
  ...
  var secret = _props.getProperty('CLOUDFLARE_BRIDGE_SECRET') || '';
  var timestampNumber = Number(timestamp);
  if (!secret || !email || !/^[0-9]{10,16}$/.test(timestamp) || !/^[A-Za-z0-9_-]{20,200}$/.test(nonce)) return {ok:false, error:'Solicitud de identidad no válida'};
  if (Math.abs(Date.now() - timestampNumber) > 60000) return {ok:false, error:'Solicitud de identidad vencida'};
  if (['ADMIN', 'AUXILIAR'].indexOf(role) === -1) return {ok:false, error:'Rol no autorizado'};
  var message = email + '\n' + role + '\n' + timestamp + '\n' + nonce;
  if (!constantTimeEqual_(cloudflareSignature_(message, secret), signature)) return {ok:false, error:'Firma de identidad no válida'};
  var cache = CacheService.getScriptCache();
  var nonceKey = 'cf_nonce_' + nonce;
  if (cache.get(nonceKey)) return {ok:false, error:'Solicitud de identidad repetida'};
  cache.put(nonceKey, '1', 120);
  var sessionToken = createSession();
  var adminData = getAdminData();
  adminData.sessionToken = sessionToken;
  adminData.identity = {email:email, role:role};
  return adminData;
}
...
function doGet(e) {
  ...
  if (p.action === 'cloudflareSession') {
    return js(createCloudflareSession_(p));
  }
